...
namespace HotelManagement.WebUILayer.Areas.Admin.Controllers
{
    ...
    public class ServiceController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> InsertService(InsertServiceDTO dto)
        {
            var client = _httpClientFactory.CreateClient();
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"http://localhost:5191/api/Service?serviceId={id}");
            if (responseMessage.IsSuccessStatusCode)
                return RedirectToAction("Index", "Service", new { area = "Admin" });
            return RedirectToAction("Index", "Service", new { area = "Admin" });
        }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"http://localhost:5191/api/Service/GetById?serviceId={id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateService(UpdateServiceDTO dto)
        {
            ...
            var responseMessage = await client.PutAsync("http://localhost:5191/api/Service", content);
            if (responseMessage.IsSuccessStatusCode)
                return RedirectToAction("Index", "Service", new { area = "Admin" });
            return View(dto);
        }
